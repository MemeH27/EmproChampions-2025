...
export default function ControlPartido() {
  ...
  // ==================================================================
  // ========= INICIO DE LA CORRECCIÓN: Estado de control =============
  // ==================================================================
  ...
  useEffect(() => {
    ...
    const listeners = [
      ...
      onValue(refs.eventos, (snap) => {
        ...
        const listaCompleta = [], listaGoles = [], listaTarjetas = [], listaCambios = [];
        for (const id in todosLosEventosObj.goles) { listaGoles.push({ ...todosLosEventosObj.goles[id], id }); listaCompleta.push({ ...todosLosEventosObj.goles[id], id }); }
        for (const id in todosLosEventosObj.tarjetas) { listaTarjetas.push({ ...todosLosEventosObj.tarjetas[id], id }); listaCompleta.push({ ...todosLosEventosObj.tarjetas[id], id }); }
        for (const id in todosLosEventosObj.cambios) { listaCambios.push({ ...todosLosEventosObj.cambios[id], id }); listaCompleta.push({ ...todosLosEventosObj.cambios[id], id }); }
    ...
    return () => {
      Object.keys(refs).forEach((key, index) => {
        if (listeners[index]) { off(refs[key], 'value', listeners[index]); }
      });
    };
  }, [partidoId]);
  ...
  // ==================================================================
  // ========= INICIO DE LA CORRECCIÓN: Función "Fortificada" =========
  // ==================================================================
  const guardarTodoEnFirebase = async (mvpSeleccionado) => {
    // 1. Control para evitar doble ejecución
    if (isSaving) {
      console.log("Guardado ya en progreso. Evitando doble ejecución.");
      return;
    }
    setIsSaving(true);
    ...
    if (!mvpSeleccionado) {
      ...
      setMostrarMVP(false);
      setIsSaving(false); // Liberamos el bloqueo
      return;
    }
    ...
    try {
      ...
      const actualizarEstadisticas = (equipo, golesAFavor, golesEnContra, resultado) => {
        if (!equipo || !equipo.nombre) return; // Validación de seguridad
        const stats = tablasActuales[equipo.nombre] || { pj: 0, pg: 0, pe: 0, pp: 0, gf: 0, gc: 0, puntos: 0, logo: equipo.logo };
        ...
        tablasActuales[equipo.nombre] = stats;
      };
      ...
      for (const gol of golesRegistrados) {
        if (!gol.jugador) continue; // Si no hay nombre de jugador, saltamos este gol
        const jugadorKey = gol.jugador.replace(/[.#$[\]]/g, '_');
        if (!goleadoresDetallesActuales[jugadorKey]) {
          goleadoresDetallesActuales[jugadorKey] = [];
        }
        const golYaExiste = goleadoresDetallesActuales[jugadorKey].some(
          (golExistente) => golExistente.idDelGol === gol.id
        );
        if (!golYaExiste) {
          const equipoGoleador = gol.equipo === "equipo1" ? equipo1 : equipo2;
          goleadoresDetallesActuales[jugadorKey].push({
            idDelGol: gol.id,
            minuto: gol.minuto,
            partidoId: partidoId,
            partidoNombre: `${equipo1?.nombre || 'Equipo 1'} vs ${equipo2?.nombre || 'Equipo 2'}`,
      ...
      await set(refHistorial, {
        fecha: new Date().toLocaleDateString("es-HN"),
        equipo1: equipo1?.nombre || 'Equipo 1',
        equipo2: equipo2?.nombre || 'Equipo 2',
        logo1: equipo1?.logo || '',
        logo2: equipo2?.logo || '',
        goles1: goles.equipo1,
        goles2: goles.equipo2,
        mvp: mvpSeleccionado?.nombre || 'No definido',
      });
      ...
      setIsSaving(false); // Liberamos el bloqueo en caso de error
    }
  };
  // ================================================================
  // ================= FIN DE LA CORRECCIÓN =========================
  ...
  if (!equipo1 || !equipo2) {
    return (
      <div className="w-full min-h-screen bg-cover bg-center text-white flex flex-col bg-main-background">
        <Navbar />
  ...
  return (
    <div className="w-full min-h-screen bg-cover bg-center text-white font-qatar px-2 md:px-8 py-4 bg-main-background flex flex-col">
        <Navbar />
